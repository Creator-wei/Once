...
def train_ssl_one_epoch(teacher_model, student_model, optimizer, labeled_loader, unlabeled_loader, epoch_id, lr_scheduler, accumulated_iter, ssl_cfg,
    ...
    ##########################################################################################
    '''
    selected_label_cls = Counter({1:1,2:1,3:1,4:1,5:1})
    selected_label_iou = Counter({1:1,2:1,3:1,4:1,5:1})
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        pseudo_counter_cls = Counter(selected_label_cls.tolist())
        if max(pseudo_counter_cls.values()) < len(ud_teacher_batch_dict):  # not all(5w) -1
            for i in range(len(ssl_cfg.CLASS_NAMES)):
