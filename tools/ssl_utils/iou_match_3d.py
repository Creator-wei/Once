...
def iou_match_3d(teacher_model, student_model,
    ...
    for i in range(len(cfgs.CLASS_NAMES)):
        #classwise_acc[i] = 1-0.5*(selected_label_cls[i+1] / sum(selected_label_cls.values()))  # 每个类别/max
    ...
    for i in range(len(cfgs.CLASS_NAMES)):
        #iouwise_acc[i] = 1-0.5*(selected_label_iou[i+1] / sum(selected_label_iou.values()))  # 每个类别/max
